...
export class ChatRepositoryAdapter implements ChatRepositoryPort {
  ...
  async create(userId: string | null, data: CreateChatDto): Promise<Chat> {
  ...
  async updateTitle(chatId: string, data: UpdateChatTitleDto): Promise<Chat> {
  ...
  async updateMetadata(chatId: string): Promise<void> {
    await this.chatRepository.update({ id: chatId }, {
