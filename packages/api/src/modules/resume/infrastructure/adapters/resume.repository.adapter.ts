...
export class ResumeRepositoryAdapter implements ResumeRepositoryPort {
  ...
  async findByUserId(userId: string): Promise<ResumeEntity | null> {
    ...
    return resume ? new ResumeEntity(resume) : null;
  }

  async findById(resumeId: string, userId: string): Promise<ResumeEntity | null> {
    const resume = await this.resumeRepository.findByIdWithRelations(resumeId, userId);
    return resume ? new ResumeEntity(resume) : null;
  }

  async create(userId: string): Promise<ResumeEntity> {
  ...
  async update(userId: string, data: UpdateResumeDto): Promise<ResumeEntity> {
    const updatedResume = await this.resumeRepository.transaction(async (tx) => {
    ...
    return new ResumeEntity(updatedResume);
  }

  async getCompletionDetails(userId: string): Promise<any> {
    const resume = await this.resumeRepository.findByUserId(userId);
    if (!resume || !resume.user) {
      throw new Error("Resumo não encontrado");
    }
